...
export function registerCoreNodes(registry, hooks) {
  ...
  // Widget Demo Node — showcases all property panel widgets
  registry.register("core/WidgetDemo", {
    title: "Widget Demo",
    color: "#8b5cf6", // violet
    icon: "cog",
    size: { w: 200 },
    ...
    outputs: [{ name: "out", datatype: "any" }],
    properties: [
      { key: "name", label: "Name", widget: "text", placeholder: "Enter name..." },
      { key: "count", label: "Count", widget: "number", min: 0, max: 100, step: 1 },
      { key: "gain", label: "Gain", widget: "slider", min: 0, max: 2, step: 0.01 },
      { key: "enabled", label: "Enabled", widget: "toggle" },
      { key: "curve", label: "Curve", widget: "select", options: ["linear", "ease", { label: "S-Curve", value: "scurve" }] },
      { key: "mode", label: "Mode", widget: "radio", options: ["fast", "balanced", "quality"] },
      { key: "tags", label: "Tags", widget: "checkbox-group", options: [
        { label: "Urgent", value: "urgent" },
        { label: "Review", value: "review" },
        { label: "Done", value: "done" },
      ]},
      { key: "color", label: "Color", widget: "color" },
      { key: "notes", label: "Notes", widget: "textarea", placeholder: "Write notes..." },
    ],
    onCreate(node) {
      node.state.name ??= "Demo";
      node.state.count ??= 10;
      node.state.gain ??= 1;
      node.state.enabled ??= true;
      node.state.curve ??= "linear";
      node.state.mode ??= "balanced";
      node.state.tags ??= ["urgent"];
      node.state.color ??= "#8b5cf6";
      node.state.notes ??= "";
    },
    onExecute(node, { getInput, setOutput }) {
